...
The correct answers are stored in a list. The teacher then inputs a student's answers into another list. Your program should:
...
Write a program that takes `X` and `Y` as input, followed by the prices of `5` items and prints "DO use coupon" if buying the coupon is beneficial, 
otherwise "DO NOT use coupon".
...
A hospital consists of multiple floors (`3`), each with a certain number of rooms (`2`) and each room has a fixed number of beds (`4`). 
Due to rising patient intake, the hospital administration wants to efficiently manage the availability of beds across all floors.
