---
pubDatetime: 2025-04-28T15:57:52.737Z
modDatetime: 2025-04-28T15:57:52.737Z

title: Scope in Functions
metadata_title: Scope in Functions - JavaScript Programming | V.Vidhya
slug: functions-scope
featured: false
# ogImage: https://user-images.githubusercontent.com/53733092/215771435-25408246-2309-4f8b-a781-1f3d93bdf0ec.png
description: Learn Functions Scope in JavaScript Programming by practicing exercises based on real-world scenarios with V.Vidhya.

order: 260
---

import Admonition from '@/components/Admonition.astro';

#### Q1.

What will be the output of the following code?

<Admonition variant="caution" title="caution">
    You have to predict the output yourself without running/executing the code.
</Admonition>

<div class="w-full my-8"></div>

```jsx
let greet = "hello";

function outerGreet() {
    let greet = "hi";

    console.log(greet);

    function innerGreet() {
        console.log(greet);
    }
}

console.log(greet);

outerGreet();
```