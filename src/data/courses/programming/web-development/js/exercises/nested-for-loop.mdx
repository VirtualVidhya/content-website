---
pubDatetime: 2025-04-28T15:57:52.737Z
modDatetime: 2025-04-28T15:57:52.737Z

title: Nested for (loop)
metadata_title: Nested For (loop) - JavaScript Programming | V.Vidhya
slug: nested-for-loop
featured: false
# ogImage: https://user-images.githubusercontent.com/53733092/215771435-25408246-2309-4f8b-a781-1f3d93bdf0ec.png
description: Learn Nested For (loop) in JavaScript Programming by practicing exercises based on real-world scenarios with V.Vidhya.

order: 180
---

#### Q1.

Let's say user is giving 2 numbers `n1` & `n2` as input.

Write a program to print the multiplication tables of all numbers between `n1` & `n2` one by one in order.

**Expected Output:**

```ansi
[2;3m// for n1: 4 & n2: 6[0m

#table of 4
4
8
12
16
20
24
28
32
36
40
#table of 5
5
10
15
20
25
30
35
40
45
50
#table of 6
6
.
.
.
```

{/* (**NOTE:** Your program should be flexible enough so that in case in future, if we want to print tables of numbers bw 4 & 15 then it can be updated very easily.) */}