---
pubDatetime: 2025-04-28T15:57:52.737Z
modDatetime: 2025-04-28T15:57:52.737Z

title: switch (Conditional Statements)
metadata_title: Switch (Conditional Statements) - JavaScript Programming | V.Vidhya
slug: switch-conditional-statement
featured: false
# ogImage: https://user-images.githubusercontent.com/53733092/215771435-25408246-2309-4f8b-a781-1f3d93bdf0ec.png
description: Learn Switch (Conditional Statements) in JavaScript Programming by practicing exercises based on real-world scenarios with V.Vidhya.

order: 80
---

#### Q1. 

Use `switch` statement to print the day of the week using a variable `day` with possible values 1 to 7.
    
Below it shows, which day each number represents.

1 = Monday, 2 = Tuesday & so on…
    
```jsx
// if the `day` variable is currently given the value 2

// expected output
Tuesday
```